...
# Sleep until time expires and automatically shutdown.

SETUP_SHUTDOWN_TIMEOUT=`getprop ro.boot.setup_shutdown`
if [ "x$SETUP_SHUTDOWN_TIMEOUT" != "x" ] && [ $SETUP_SHUTDOWN_TIMEOUT -gt 0 ]; then
    sleep $SETUP_SHUTDOWN_TIMEOUT
    echo Power off the phone due to shutdown flag is set -- Shutdown
    setprop sys.powerctl shutdown,setup_timeout
fi
